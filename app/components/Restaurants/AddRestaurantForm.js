...
export default function AddRestaurantForm(props) {
  ...
  const addRestaurant = () => {
    if (!restaurantName || !restaurantAddress || !restaurantDescription) {
      ...
      toastRef.current.show("Se necesita la localización del restaurante");
    } else {
      setIsLoading(true);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (statusPermissions !== "granted") {
        toastRef.current.show("Se requieren permisos de localización", 3000);
      } else {
...
function UploadImage(props) {
  ...
  const imageSelect = async () => {
    ...
    if (resultPermission.status === "denied") {
      ...
      if (result.cancelled) {
        toastRef.current.show("Acción cancelada", 3000);
      } else {
  ...
  const removeImage = (image) => {
    Alert.alert(
      "Eliminar Imagen",
      "¿Está seguro que quiere eliminar la imagne?",
      [
        {
          text: "Cancel",
          style: "cancel",
